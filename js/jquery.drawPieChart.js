...
function extend($, undefined) {
	$.fn.drawPieChart = function (data, options) {
		...
		for (var i = 0, len = data.length; i < len; i++) {
			...
			p.setAttribute("opacity", data[i].value / segmentTotal);
		...
		function pathMouseEnter(e) {
			...
			var ratio = Math.round(100 * data[index].value / segmentTotal);
			$tip.text(data[index].title + ": " + ratio + "%").fadeIn(200);
